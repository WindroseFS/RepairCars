...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    suspend fun getAllContacts(): List<Contact> = withContext(Dispatchers.IO) {
        ...
        readableDatabase.rawQuery("SELECT id, name FROM contacts", null).use { cursor ->
            while (cursor.moveToNext()) {
                contacts.add(Contact(cursor.getLong(0), cursor.getString(1)))
            }
        }
        contacts
    }
    ...
    suspend fun getContactsWithLastMessage(): List<ContactDisplay> = withContext(Dispatchers.IO) {
        ...
        readableDatabase.rawQuery(
            ...
            while (cursor.moveToNext()) {
                ...
                val lastMessage = if (cursor.isNull(2)) "Sem mensagens" else cursor.getString(2)
                list.add(ContactDisplay(id, name, lastMessage))
            }
        }
        list
    }
    ...
    suspend fun addMessage(contactId: Long, text: String, isSentByMe: Boolean, lat: Double? = null, lng: Double? = null): Long = withContext(Dispatchers.IO) {
